...
public class LinearProbingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        if (key == null || value == null) {
            throw new IllegalArgumentException("Both key and value can not be null.");
        }
        double lf = (size + 1.0) / table.length;
        ...
        while (true) {
            if (table[index] == null) {
                LinearProbingMapEntry<K, V> newHash = new LinearProbingMapEntry<>(key, value);
                if (firstRemoved != -1) {
                    index = firstRemoved;
                }
                table[index] = newHash;
                ...
                V returnVal = table[index].getValue();
    ...
    public V remove(K key) {
        ...
        throw new NoSuchElementException("Key does not exist in this map.");
    }
    ...
    public boolean containsKey(K key) {
        if (key == null) {
            throw new IllegalArgumentException("The key you are looking for is null.");
        }
        int index = key.hashCode() % table.length;
        while (table[index] != null) {
            if (table[index].getKey().equals(key)) {
    ...
    public void resizeBackingTable(int length) {
        if (length < size) {
            throw new IllegalArgumentException("Length of array is smaller than number of items in hash map.");
        }
